...
public extension Renderer2D {
	...
	@inlinable static func texture(_ value: some Texture, at x: Int, _ y: Int, tint: Color = Renderer.tint) {
		DrawTexture(value.rawValue, x.toInt32, y.toInt32, tint.rawValue)
	}
	
	@inlinable static func texture(_ value: some Texture, at position: Vector2, tint: Color = Renderer.tint) {
		DrawTextureV(value.rawValue, position.rawValue, tint.rawValue)
	}

	//MARK: - Atlas
	
	@inlinable static func atlas(_ value: some Texture, from source: Rectangle, at position: Vector2, tint: Color = Renderer.tint) {
		DrawTextureRec(value.rawValue, source.rawValue, position.rawValue, tint.rawValue)
	}

	@inlinable static func atlas(_ value: some Texture, from source: Rectangle, to destination: Rectangle, origin: Vector2, rotation: Float = 0, tint: Color = Renderer.tint) {
		DrawTexturePro(value.rawValue, source.rawValue, destination.rawValue, origin.rawValue, rotation, tint.rawValue)
	}
	
	@inlinable static func atlas(_ value: some Texture, from source: Rectangle? = nil, to destination: Rectangle, rotation: Angle = .zero, tint: Color = Renderer.tint) {
	...
	@inlinable static func target(_ value: RenderTexture, to destination: Rectangle, rotation: Angle = .zero, tint: Color = Renderer.tint) {
