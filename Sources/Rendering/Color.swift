...
public extension Color {
	...
	@inlinable static func rgba(_ value: UInt32) -> Color {
		self.init(rawValue: GetColor(value))
	}

	@inlinable static func rgb(_ r: UInt8, _ g: UInt8, _ b: UInt8, a: UInt8 = .max) -> Color {
	...
	@inlinable func faded(to alpha: Float) -> Color {
		Color(rawValue: Fade(rawValue, alpha))
	}
	
	@inlinable mutating func fade(to alpha: Float) {
		self = faded(to: alpha)
	}
